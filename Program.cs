    ...
    class MPIProgram {
		...
		public static void Main (string[] args) {
			...
			if (MPI.IAm == 2) {
				...
				//
				scatterable = new();
				string scat = "Scattering: ";
				for (int ndx = 0; ndx <= MPI.NodeCount; ndx++) {
					scatterable.Add (ndx * 10 + 0);
					scatterable.Add (ndx * 10 + 1);
					scat += $"{ndx * 10} {ndx * 10 + 1} ";
				}
				MPI.Print(scat);
				Collective.Scatter<int>(2, scatterable);
			} else {
				scatterable = Collective.Scatter<int>(2, null);
			}
			...
			// nodes in "node order."
			//
			string scattered = string.Empty;
			foreach (int i in scatterable)
				scattered += $"{i} ";
			MPI.Print(scattered);
			...
			List<int> gathered = Collective.Gather<int>(1, scatterable);
			if (MPI.IAm == 1) {
